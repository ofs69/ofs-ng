...
Every user-visible string lives once in `localization/strings.toml` (the English source of truth) and
is reached through the generated `Str::`/`Tr` API — see the *Localized strings* section of
...
| Path | Status |
|------|--------|
| `lang/<id>.toml` | **Shipped.** Strictly validated by the build (`localization_gen --validate`). A file here must be complete: every source key present with a non-empty translation whose `{N}` placeholders match the source. An incomplete file here **fails the build.** |
| `localization/wip/<id>.toml` | **Work in progress.** Not globbed by the build, not shipped, may be partially translated. New languages are seeded here and live here until complete. |

A language is `promote`d from `wip/` to `lang/` only once it is 100 % complete and valid, so the build
stays green while a translation is underway.
...
build for every shipped catalog that translated it, until each is refreshed. `python
tools/translations.py check` reproduces these exact checks without a build (use it in CI or pre-commit).
...
(re-stamps the `english` reference, clearing the stale flag). Work-in-progress files under
`localization/wip/` are exempt — they are not built — so a half-finished language never blocks the build.
...
python tools/translations.py new    <code>     # seed a wip stub for a new language
...
python tools/translations.py promote <id>      # move a complete wip language into lang/
```
...
the one command that replaces hand-editing every language file. A `sync` that adds keys to a *shipped*
`lang/` file warns you — that language is now incomplete and will fail the build until completed or
moved back to `wip/`.
...
1. `python tools/translations.py todo de_[AI]` — writes `localization/wip/de_[AI].batch.toml` containing
   only the keys that need work, each with its English text, translator `# context:` (the source
   `description`), and `# placeholder:` docs.
...
python tools/translations.py promote de_[AI]   # wip/ -> lang/  (refuses if incomplete)
```

After `promote`, build and run the suite — the `ui-smoke-loc` test re-runs the whole UI under a
...
The seeded set (under `localization/wip/`) targets the largest user bases for a tool like this:
Simplified Chinese, Spanish, German, French, Russian, Brazilian Portuguese, Korean, Italian, Polish,
Turkish, and Traditional Chinese. Japanese already ships. Add any other with `new`.
