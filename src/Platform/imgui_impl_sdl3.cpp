...
//  2026-03-25: [Docking] Use SDL_HAS_EVENT_DISPLAY_USABLE_BOUNDS_CHANGED (SDL 3.4.0+) when available to avoid
//  refreshing monitor work area every frame on Windows. (#8415) 2026-03-09: [Docking] Fixed an issue dated 2025/04/09
//  (1.92 WIP) where a refactor+merge caused ImGuiBackendFlags_HasMouseHoveredViewport to never be set, causing foreign
//  windows to be ignored when deciding of hovered viewport. (#9284) 2026-02-13: Inputs: systems other than X11 are back
//  to starting mouse capture on mouse down (reverts 2025-02-26 change). Only X11 requires waiting for a drag by default
//  (not ideal, but a better default for X11 users). Added ImGui_ImplSDL3_SetMouseCaptureMode() for X11 debugger users.
//  (#3650, #6410, #9235) 2026-01-15: Changed GetClipboardText() handler to return nullptr on error aka clipboard
//  contents is not text. Consistent with other backends. (#9168) 2025-11-05: Fixed an issue with missing characters
//  events when an already active text field changes viewports. (#9054) 2025-10-22: Fixed Platform_OpenInShellFn()
//  return value (unused in core). 2025-09-24: Skip using the SDL_GetGlobalMouseState() state when one of our window is
//  hovered, as the SDL_EVENT_MOUSE_MOTION data is reliable. Fix macOS notch mouse coordinates issue in fullscreen mode
//  + better perf on X11. (#7919, #7786) 2025-09-18: Call platform_io.ClearPlatformHandlers() on shutdown. 2025-09-15:
//  Use SDL_GetWindowDisplayScale() on Mac to output DisplayFrameBufferScale. The function is more reliable during
//  resolution changes e.g. going fullscreen. (#8703, #4414) 2025-06-27: IME: avoid calling SDL_StartTextInput() again
//  if already active. (#8727) 2025-05-15: [Docking] Add Platform_GetWindowFramebufferScale() handler, to allow varying
//  Retina display density on multiple monitors. 2025-05-06: [Docking] macOS: fixed secondary viewports not appearing on
//  other monitors before of parenting. 2025-04-09: [Docking] Revert update monitors and work areas information every
//  frame. Only do it on Windows. (#8415, #8558) 2025-04-22: IME: honor ImGuiPlatformImeData->WantTextInput as an
//  alternative way to call SDL_StartTextInput(), without IME being necessarily visible. 2025-04-09: Don't attempt to
//  call SDL_CaptureMouse() on drivers where we don't call SDL_GetGlobalMouseState(). (#8561) 2025-03-30: Update for
//  SDL3 api changes: Revert SDL_GetClipboardText() memory ownership change. (#8530, #7801) 2025-03-21: Fill gamepad
//  inputs and set ImGuiBackendFlags_HasGamepad regardless of ImGuiConfigFlags_NavEnableGamepad being set. 2025-03-10:
//  When dealing with OEM keys, use scancodes instead of translated keycodes to choose ImGuiKey values. (#7136, #7201,
//  #7206, #7306, #7670, #7672, #8468) 2025-02-26: Only start SDL_CaptureMouse() when mouse is being dragged, to
//  mitigate issues with e.g. Linux debuggers not claiming capture back. (#6410, #3650) 2025-02-25: [Docking] Revert to
//  use SDL_GetDisplayBounds() for WorkPos/WorkRect if SDL_GetDisplayUsableBounds() failed. 2025-02-24: Avoid calling
//  SDL_GetGlobalMouseState() when mouse is in relative mode. 2025-02-21: [Docking] Update monitors and work areas
//  information every frame, as the later may change regardless of monitor changes. (#8415) 2025-02-18: Added
//  ImGuiMouseCursor_Wait and ImGuiMouseCursor_Progress mouse cursor support. 2025-02-10: Using SDL_OpenURL() in
...
//  SDL_EVENT_MOUSE_WHEEL event doesn't require dividing by 100.0f on Emscripten. 2024-09-11: (Docking) Added support
//  for viewport->ParentViewportId field to support parenting at OS level. (#7973) 2024-09-03: Update for SDL3 api
...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event *event) {
    ...
    case SDL_EVENT_KEY_UP: {
        ...
        if (viewport == nullptr)
            return false;
        // IMGUI_DEBUG_LOG("SDL_EVENT_KEY_%s : key=0x%08X ('%s'), scancode=%d ('%s'), mod=%X, windowID=%d,
        // viewport=%08X\n",
        //     (event->type == SDL_EVENT_KEY_DOWN) ? "DOWN" : "UP  ", event->key.key, SDL_GetKeyName(event->key.key),
        //     event->key.scancode, SDL_GetScancodeName(event->key.scancode), event->key.mod, event->key.windowID,
    ...
    case SDL_EVENT_WINDOW_FOCUS_LOST: {
        ...
        // "SDL_EVENT_WINDOW_FOCUS_GAINED" : "SDL_WINDOWEVENT_FOCUS_LOST", event->window.windowID, viewport ?
        // viewport->ID : 0);
